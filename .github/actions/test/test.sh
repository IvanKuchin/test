...
source "${GITHUB_WORKSPACE}/.github/lib/log.sh" # Source the function script(s)
...
ValidateInput() {
  ...
  ########################################
  # See if we need values for Docker hub #
  ########################################
  ...
  if [ -z "${IMAGE_REPO}" ]; then
    ...
    warn "Image repo set to GitHub repo name :${F[W]}[IMAGE_REPO]${F[B]}, value:${F[W]}[${IMAGE_REPO}]"
  fi
...
RunSASTDocker() {
  info "-------------------------------------------------------"
  info "RunSASTDockerImage"
  info "-------------------------------------------------------"

  docker run -t --rm -v /home/runner/work/test/test/src:/code -v /var/run/docker.sock:/var/run/docker.sock  registry.gitlab.com/gitlab-org/security-products/sast:latest /app/bin/run /code
}
...
# Upload the image #
####################
### commented due to upload needed only on releases
# UploadImage
